...
Note: Use the customize function to display the information window.

## Launch web resources

Launching the contact's web resources is now also possible on the vendor / customer card and also on related documents (quotation, order, invoice, cash receipt, payment order). To start, click on the CreditCheck status icon (traffic light).
...
The system finds sales line discounts in the following order:
1.	Campaign and Goods
2.	Campaign and Price group of goods
3.	Customer and Goods
...
2.	Create a new sales invoice according to your habits. In the Due date field, define the latest da-te for the payment calendar.
...
2.	Select the number of the bank with the set payment accumulation for which you want to crea-te a payment order and confirm with the **OK** button.
...
The receivable collection status is displayed in the Customer Item Details Information window. As a result, the user has immediate access to this basic information, but also detailed information at the click of a button.
...
3.	Confirm that you want to create a receivables information card.
...
For more information, visit Number Series Links Setup. [Controling Basic - Setup](ac-controling-basic-setup.md). 
...
For more information, visit Rounding setup according to the method of payment.
...
1. Choose the ![Lightbulb that opens the Tell Me feature.](media/ui-search/search_small.png "Tell me what you want to do") icon, enter **Config.Wksh. - Data Structure**, and then choose the related link. 
2.	Select Package Code.
3.	Select the **OK** button to run the report.
 

### Finance 

-   **Reports in Item Journal**

    The following reports have been added to the **Item Registers** page:
...
- **Banks** 
    ...
    > [!Note]
    > As of version BC19, it is recommended to use the newer Banking Documents functionality, which does not use the Reconciliation Log.
...
- **QR code for payment on the sales invoice ** 
    ...
    > [!NOTE]
    > In BC online it is necessary to use different method – see  [Barcode Fonts with Business Central Online - Business Central | Microsoft Docs](https://docs.microsoft.com/en-us/dynamics365/business-central/dev-itpro/developer/devenv-report-barcode-fonts#one-dimensional-barcode-fonts-with-business-central-online) 
...
- **Creating web contact resources** 
    ...
    1.	Choose the ![Lightbulb that opens the Tell Me feature.](media/ui-search/search_small.png "Tell me what you want to do") icon, enter **Create Contact Web Sources**, and then choose the related link.
    2.	In the options, select the Web Source Code you want to add to Contacts.
    3.	In the options, select the contact field from which the variable value of the link URL is to be filled (Search Word Field).
    4.	If necessary, specify a filter to restrict the selection of contacts to which the above should be applied.
    5.	Select the **OK** button to run the report.


## Reduciton of VAT deduct

The goal of the functionality is to enable additional deduction of the reduction of the VAT deduction from VAT accounts to defined financial accounts (usually cost accounts). According to the registered estimate (advance coefficient), VAT deductions are charged during the accounting period and. If necessary, after the end of the calendar year, it is possible to recalculate according to the actual share of use of these taxable supplies for the purposes of the performed transactions in the given year.

With the right settings, this functionality is suitable not only for Czech but also for Slovak companies.

> [!Note]
> Immediate billing of "non-deductible" VAT on primary documents (invoice, credit note, cash document, financial journal) or routing of "non-deductible" VAT for the source of taxation (goods, fixed assets, service) is not addressed.

**Function Calculate and post VAT coefficient**

This function allows you to prepare a print report, or and post calculated VAT deductions. The function only works with VAT items containing combinations of account groups set up for VAT deductions - see Setting the VAT deduction reduction.
...
2.	In Options, fill in:
    ...
    - Starting date – the first date in the period from which VAT entries are processed in the batch job.
    - Ending date
    - Starting VAT date date – the first date in the VAT period from which VAT entries are processed in the batch job.
    - Ending VAT date
    ...
    - Dim. values from coeff. cost acc. – whether to take dimension values from the coefficient cost account.
